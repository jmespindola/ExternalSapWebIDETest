sap.ui.define([
	...
	return BaseObject.extend("capu.model.GroupSortState", {
		 ...
		 * @alias capu.model.GroupSortState
		 */
		...
		/**
		 * Sorts by U_ModDesc, or by U_Price
		 *
		...
		/**
		 * Groups by U_Price, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "U_Price") {
				...
				this._oViewModel.setProperty("/sortBy", "U_Price");

				aSorters.push(
					new Sorter("U_Price", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "U_ModDesc");
			}
